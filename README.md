...
The Proxy supports a filter plugin system that allows you to intercept and modify Kafka requests and responses.
Plugins are implemented as separate Go binaries that communicate with the proxy over RPC.
To use filter plugins, compile them and place the binaries in a directory, then start the proxy with the `--plugin-dir` flag pointing to that directory.

The proxy will load all executable files found in the specified directory.
*   **Loading Order**: Plugins are loaded in alphabetical order of their filenames.
*   **Request Chain**: Filters are applied to requests in the order they are loaded.
*   **Response Chain**: Filters are applied to responses in the reverse order.
...
### Help output
    ...
    Flags:
          ...
          --dynamic-sequential-max-ports uint16                  If set to non-zero, ports are allocated sequentially from the half open interval [dynamic-sequential-min-port, dynamic-sequential-min-port + dynamic-sequential-max-ports)
          --dynamic-sequential-min-port uint16                   If set to non-zero, makes the dynamic listener use a sequential port starting with this value rather than a random port every time.
      ...
      -h, --help                                                 help for server
          ...
          --plugin-dir string                                    Directory containing filter plugins
